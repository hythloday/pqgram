...
class PQGramRegressions extends FlatSpec with Matchers {
  ...
  "labeltuple union" should "takes the min-union" in {
    val ops = PqOps[Label]()
    val lt1 = ops.extend(arg0, p, q).subgraphs.map(g => ops.labelTuple(g, g.root)).map(_.map(_.i).mkString("-"))
    val lt2 = ops.extend(arg1, p, q).subgraphs.map(g => ops.labelTuple(g, g.root)).map(_.map(_.i).mkString("-"))

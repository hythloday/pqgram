...
class PQGramRegressions extends FlatSpec with Matchers {
  ...
  implicit val boundedLabel = new Bounded[Label] {
    override val infimum: Label = new Label(UUID.randomUUID().toString, Int.MinValue)
    override val supremum: Label = new Label(UUID.randomUUID().toString, Int.MaxValue)
    override def compareInner(x: Label, y: Label): Int = x.i compare y.i
  }



//  val arg0 = MultiDAG.from(List(
